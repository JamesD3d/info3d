...
3D printing is an exciting technology that is very engaging for students of all ages. 
When establishing a 3D printing program in the classroom, it is important to consider the following:

## Time:
- Depending on the printer, most small to medium-sized objects can take between 20 minutes and 2 hours.
...
If this instructor has two of these classes, it is evident that the amount of time needed is extensive.
...
- Depending on the size of the printer, the model size, and the area needed for supports, attempt to put more than one model on the print bed. This can decrease the amount of time between model completions, but it will increase the time needed to complete a job and also increase the chance of failure.
...
<Callout type="default">
  The Windows app [3D Viewer](https://apps.microsoft.com/detail/9nblggh42ths?hl=en-US&gl=US) is a great tool to view 3D models in the file explorer preview panel.
</Callout>
...
<Steps>
### Google Form
- To manage my classroom projects, I selected Google Forms. This was a simple form that recorded the student's email address, class period, grade level, and did not allow students to submit the incorrect file format.

### Label Files
- Ensure that students label their downloaded model files with their names and class periods.

### Increase File Size Limit
- Ensure that the file size limit is increased to allow for larger files.

### Viewing Submissions
- Add a checkbox column before the student's email address.
> Many districts have the option to convert student emails to "cards" with a friendly name.

### Download the Models
- When you begin to print the student .stl files, you may choose to download individual files via the link provided in Google Sheets, or you may download all the files at once by downloading a .zip file of the Google folder associated with the form.

### Slice the Models
- Once all the files are downloaded, within slicers like Bambu Studio or Orca Slicer, you can load all the models at once. The slicer can auto-arrange all the class's models into an efficient layout and arrangement on multiple build plates. The files can then be sliced and printed in sequence.

### Remove supports and clean up the models as needed.
<Callout type="warning">
  Be careful when allowing students to remove supports. Small pieces of plastic may break off violently and cause injury. I recommend eye protection when removing supports.
</Callout>
</Steps>
